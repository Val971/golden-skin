  ...
  return (
    <>
      {Array.from({ length }).map((_, index) => (
        <div key={index} role='status' className='max-w-sm rounded-lg '>
          ...
          <div className=' w-full flex justify-between items-start animate-pulse'>
            <div className='block'>
              <h3 className={`h-3 bg-gray-300 rounded-full mb-4 ${widht}`}></h3>
              <p
                className={`h-2 bg-gray-300 rounded-full mb-2.5 ${whidht2}`}></p>
            </div>
            <span className='h-2 hidden bg-gray-300 rounded-full w-16 '></span>
          </div>
        </div>
      ))}
    </>
