...
export default function FamousProductList() {
  ...
  if (isLoading) {
    return (
      <div className='container mx-auto px-4 sm:px-6 lg:px-8 grid 2xl:grid-cols-5 xl:grid-cols-4 lg:grid-cols-3 grid-cols-2 md:gap-7 mt-20'>
        {Array.from({ length: 4 }).map((_, index) => (
          <div key={index} role='status' className='max-w-sm  rounded-lg p-4 '>
            ...
            <div className=' w-full flex justify-between items-start animate-pulse'>
              <div className='block'>
                <h3 className='h-3 bg-gray-300 rounded-full w-32 md:w-48 mb-4'></h3>
                <p className='h-2 bg-gray-300 rounded-full w-32 mb-2.5'></p>
              </div>
              <span className='h-2 bg-gray-300 rounded-full hidden md:w-16 '></span>
            </div>
          </div>
        ))}
        {/* <SkeletonCard
